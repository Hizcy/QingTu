$(document).ready(function(){
    ...
    var $bpwd = $(".bpwd"),
        ...
        //实现placeholder   
        $(".bpwd,.brpwd,.bname,.bverify,.bphone,.bemail").focus(function(){
        ...
        $bemail.next().click(function(){
            clickText($bemail,$(this));
        });
        ...
        //检查邮箱是否合法
        function checkEmail(){
            var $bemailVal = $bemail.val();
            var $nextp = $(".bemail_note");
            $bemailVal = Trim($bemailVal,"g");
            if(!$emailpreg.test($bemailVal)){
                WrongColor($nextp,$bemail);
                $nextp.html("请填写准确的邮箱");
                return false;
            }else{
                $nextp.html("&nbsp;");
                RightColor($nextp,$bemail);
                return true;
            }    
        }
