...
function preselect(p_key,c_key,t_key) {
	document.getElementById("s1").value = p_key;
    change(1);
	document.getElementById("s2").value = c_key;
	change(2);
	document.getElementById("s3").value = t_key;
}

function preselectA(p_key,c_key,t_key) {
	document.getElementById("p1").value = p_key;
    changeA(1);
	document.getElementById("p2").value = c_key;
	changeA(2);
	document.getElementById("p3").value = t_key;
}

function changeA(v) {
    var str = "0";
    for (i = 0; i < v; i++) { str += ("_" + (document.getElementById(p[i]).selectedIndex - 1));}
    ;
    var pp = document.getElementById(p[v]);
    with (pp) {
        length = 0;
        options[0] = new Option(opt0[v], opt0[v]);
        if (v && document.getElementById(p[v - 1]).selectedIndex > 0 || !v) {
            if (dsy.Exists(str)) {
                ar = dsy.Items[str];
                for (i = 0; i < ar.length; i++)options[length] = new Option(ar[i], ar[i]);
                if (v)options[0].selected = true;
...
function setupA() {
    ...
    document.getElementById(p[i]).onchange = new Function("changeA(" + (i + 1) + ");");
    changeA(0);
}
